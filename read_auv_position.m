...
global range;
S = load('mr_data_5.mat');
% plot(S.data(:,2),S.data(:,5))
% figure
% plot(S.data(:,2),S.data(:,6))
% figure
% plot3(S.data(:,6),S.data(:,5),S.data(:,1))
% grid on
% figure
% plot3(S.data(:,6),S.data(:,5),S.data(:,7))
% grid on
global SIMULATION;
SIMULATION=0;
...
POSITION_PLOT=0; % 1 to see the positions else 0
...
P_cov=blkdiag(1*eye(3),.0024*eye(3),.0024*eye(3*no_of_links),0.01*eye(3),0.0001*eye(3),0.0001*eye(3*no_of_links));
...
initial_time_gps=S.data(1)-1480093200+17*3600;
...
if SIMULATION==0
    ...
    while (~feof(fileID))                               % For each block:
        ...
        % %
        %     InputText = textscan(fileID,'SOU_FFID %f')     % Read the numeric value
        ...
        out_mat(i,1)=hour(i)*3600+minute(i)*60+second(i)-(hour(1)*3600+minute(1)*60+second(1));
...
disp('initial_time_gps-out_mat(1,4)')
    ...
    %FFID	CHAN	SOU_X	SOU_Y	REC_Y	REC_X	range(ms)_mobSou	range(ms)_staticSou
    for i=1:8
        range{i}.mob=[];
        range{i}.mob_m=[];
        range{i}.stat=[];
    ...
    for i=1:size(MR_ranges,1)
        ...
        range{MR_ranges(i,2)}.stat_m=[range{MR_ranges(i,2)}.stat_m;MR_ranges(i,8)/1000*sound_speed];
        index=find(out_mat(:,3)==MR_ranges(i,1));
        time_stamp=out_mat(index,1);
...
%     end
    ...
    taux=0.02*(roll_des(i+1)-q(4,i))-0.0*qdot(4,i); % roll control
    tauy=5*(pitch_des(i+1)-q(5,i))-20*qdot(5,i); % pitch control
    tauz=5*(yaw_des(i+1)-q(6,i))-20*qdot(6,i); % yaw control
    
    %%%%%%%%%%%% translational motion control %%%%%%%%%%%%%%%%%%%%
    fx_i=10*(pos_des_x(i+1)-q(1,i));%-3*qdot(1,i); % x control
    fy_i=10*(pos_des_y(i+1)-q(2,i));%-3*qdot(2,i); % y control
    fz_i=10*(pos_des_z(i+1)-q(3,i));%-3*qdot(3,i); % z control
...
if ERR_PLOT==1
    ...
    if no_of_links>0
        for i=1:no_of_links
            ...
            thisax = subplot(2,1,2);
            plot(t(1:samples),Xe((row/2+6+1+3*(i-1)):(row/2+6+3*i),1:col-1),'LineWidth',2)
...
if ACT_DATA_PLOT==1
    ...
    if no_of_links>0
        for i=1:no_of_links
            ...
            thisax = subplot(2,1,2);
            plot(t(1:samples),X((row/2+6+1+3*(i-1)):(row/2+6+3*i),1:col-1),'LineWidth',2)
...
if EST_DATA_PLOT==1
    ...
    figure
    thisax = subplot(2,1,1);
    plot(t(1:samples),Xhat(1:3,1:col-1),'LineWidth',2)
    legend(thisax,'X-dir','Y-dir','Z-dir', 'location', 'northeast')
    xlabel('$t [s]$','Interpreter','LaTex','FontSize',16);
    ...
    thisax = subplot(2,1,2);
    plot(t(1:samples),Xhat(row/2+1:row/2+1+2,1:col-1),'LineWidth',2)
    legend(thisax,'X-dir','Y-dir','Z-dir', 'location', 'northeast')
    xlabel('$t [s]$','Interpreter','LaTex','FontSize',16);
    ...
    figure
    thisax = subplot(2,1,1);
    plot(t(1:samples),Xhat(4:6,1:col-1),'LineWidth',2)
    ...
    thisax = subplot(2,1,2);
    plot(t(1:samples),Xhat(row/2+1+3:row/2+1+5,1:col-1),'LineWidth',2)
    ...
    if no_of_links>0
        for i=1:no_of_links
            ...
            plot(t(1:samples),Xhat(7+3*(i-1):6+3*i,1:col-1),'LineWidth',2)
            ...
            thisax = subplot(2,1,2);
            plot(t(1:samples),Xhat((row/2+6+1+3*(i-1)):(row/2+6+3*i),1:col-1),'LineWidth',2)
