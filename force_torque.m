...
for i=no_of_links:-1:1
    ...
    v_rel_pi_i=-water_speed+V(:,2+i); % relative velocity
    v_ax_pi_i=dot(x_cap_pi_i,v_rel_pi_i)*x_cap_pi_i; % axial component of the relative speed
    v_or_pi_i=v_rel_pi_i-v_ax_pi_i; % orthogonal component of the relative velocity
    
    
    fdrag_pi_i=(-fric_coeff_link*v_ax_pi_i-0.5*density_w*norm(v_or_pi_i)*v_or_pi_i*surf_area_link_side*lift_coeff_link...
        -0.5*density_w*norm(v_ax_pi_i)*v_ax_pi_i*surf_area_link_front*drag_coeff_link); % linear drag force (links)
    ...
    fb_pi_i=-buoyancy_max_link*g; % buoyancy of links
    fg_pi_i=m_curr*g; % gravitational force of the links
...
Rc=R(:,:,1); % rotation matix of the centroid
x_cap=Rc(:,1); % unit vector in the axial direction

v_rel=-water_speed+V(:,1); % relative speed
v_ax=dot(x_cap,v_rel)*x_cap; % axial component of the relative speed
v_or=v_rel-v_ax; % orthogonal component of the relative speed

fdrag=(-fric_coeff*v_ax-0.5*density_w*norm(v_ax)*v_ax*surf_area_auv_front*drag_coeff...
    -0.5*density_w*norm(v_or)*v_or*surf_area_auv_side*lift_coeff); % translational drag
...
fb=-buoyancy_max_auv*g; % buoyancy of the auv
fg=m_auv*g; % gravitational force of the auv
